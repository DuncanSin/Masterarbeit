...
Zu den Bibliotheken die NEON-optimierten FFT-Code bereitstellen, gehören unter anderem \textit{"`Fastest Fourier Transformation in the West"'}, \textit{"`Fastest Fourier Transformation in the South"'} , \textit{"`Cricket FFT"'} und \textit{"`Libav"'} (\textbf{\ref{subsec:libav}}). Da jede einzelne von sich selbst behauptet den schnellsten FFT-Code zu beinhalten, aber meistens keine Benchmarks vorhanden sind, wurde für diese Arbeit \textit{"`Libav"'} gewählt, da dieses die weitverbreiteste Bibliothek ist und auch in namenhaften Programmen, wie zum Beispiel dem \textit{VideoLan Player} eingesetzt wird \cite{vlc}.
...
Da darüber hinaus in Libav mehrere FFTs zur Verfügung gestellt werden, die für ganz unterschiedliche Systeme optimiert wurden, soll in diesem Kapitel nur auf den für ARM-Prozessoren optimierten NEON-Code eingegangen werden. 

\subsubsection{Aufbau der FFT}\label{subsubsec:libavfft}

Als erstes soll hier erwähnt werden, das Libav zwei Ansätze einer NEON-optimierten FFT zur Verfügung stellt. Neben der in dieser Arbeit verwendeten floatingpoint Version ist auch noch eine fixpoint Version verfügbar.\\
