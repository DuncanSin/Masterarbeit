...
\subsection{Optimierung der FFT}\label{sec:optFFT}
...
Zu den Bibliotheken die NEON-optimierten FFT-Code bereitstellen, gehören unter anderem \textit{"`Fastest Fourier Transformation in the West"'}, \textit{"`Fastest Fourier Transformation in the South"'} , \textit{"`Cricket FFT"'} und \textit{"`Libav"'} (\textbf{\ref{subsec:libav}}).In dieser Arbeit \textit{"`Libav"'} gewählt, da dieses die weitverbreiteste Bibliothek ist und auch in namenhaften Programmen, wie zum Beispiel dem \textit{VideoLan Player} eingesetzt wird \cite{vlc}.

\subsubsection{Libav}\label{subsec:libav}
...
\paragraph{Aufbau der FFT}\label{subsubsec:libavfft}$\;$ \\
...
\paragraph{Einbindung in den Code}\label{subsubsec:libavin}$\;$ \\
