...
\section{Optimierung des DSP}\label{sec:optdsp}
Nachdem im vorherigen Kapitel die Optimierung des Musikklassifikators auf dem ARM Cortex-A8 beschrieben wurde, behandelt dieses Kapitel die Optimierung der Extraktionsphase auf dem C674x DSP von Texas Instruments.\\
...
Die Akkumulation innerhalb der Wurzelberechnung wurde zusätzlich für eine bessere Ausführung durch SPLOOP aufgetrennt.\\
...
Die Einbindung der DSPLIB-FFT geschieht ebenfalls auf die in \cite{rfft} beschriebene Weise. Des weiteren wurde die Übergabe zwischen FFT und MAG geändert, so dass nicht wie vorher je ein Signal für Real- und Imaginärteil übergeben wird, sondern ein einzelnes Signal in dem Real- und zugehöriger Imaginärteil hintereinander stehen. \\
...
Nach dieser Messung ergeben sich zwei neue Bottlenecks, Octave Spectral Contrast und Zero Crossing Rate. Bei OSC nimmt mit 55\% die Sortierung den größten Anteil der Laufzeit ein. Auch bei der DSP-Implementierung soll keine Optimierung in diese Richtung vorgenommen werden (vgl. \textbf{Kapitel \ref{subsec:dspwopt}}).
...
Auch nach der DSP-Optimierung wird ein abschließender Vergleich der Anfangsmessung mit der volloptimierten Messung durchgeführt. Für diesen Vergleich werden die Laufzeiten der einzelnen Features auf die Laufzeit für ein Fenster heruntergerechnet, um diesen Vergleich zwischen den verschiedenen FeatureSets zu ermöglichen.\\
%
\begin{figure}[h]
	\centering
		\includegraphics[width=1\textwidth]{../Pictures/enddsp.pdf}
	\caption{Vergleich der Laufzeiten von Referenz- und volloptimierter Messung auf dem DSP}
	\label{fig:enddsp}
\end{figure} 
%
Wie \textbf{Abbildung \ref{fig:enddsp}} zu entnehmen ist, wurde bei der Optimierung auf dem C674x DSP der größte Speedup bei der Magnitude of Spectrum erreicht, dieser beträgt knapp 29. Der geringste Speedup wurde beim Spectral Flux erzielt, dieser liegt nur knapp über 1. Die anderen Algorithmen erzielten einen Speedup zwischen 1,3 (Root Mean Square) und 7,7 (Zero Crossing Rate).   