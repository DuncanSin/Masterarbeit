...
\label{ch:grund}
\rm
In diesem Kapitel werden die Grundlagen wiederholt, die für das Verständnis der Arbeit benötigt werden.\\
Hierfür wird das Prinzip der Musikklassifikation von Musikstücken erklärt. Dabei wird ein Exkurs über Merkmale und Prozessierungen während der Musikklassifikation gegeben. Abschließend werden die vier verwendeten Musikklassifikationsmethoden erläutert.
...
\begin{figure}[htp]
	...
	\caption{Ablaufdiagramm der Musikklassifikation}
	\label{fig:mcl}
\end{figure}
%
...
Zur Extraktion von Merkmalen wird eine zuvor festgelegte Zusammenstellung von Merkmalen (engl. Feature Sets) verwendet. hierbei kann zwischen Merkmalen aus der Zeit-Domäne und Merkmalen aus der Frequenz-Domäne unterscheiden werden. Das Audio-Signal wird in gleichgroße Fenster eingeteilt, die sich überlappen können. Außerdem wird nur ein Audio-Kanal verwendet. Bei Stereo-Signalen muss daher entweder ein Kanal selektriert oder beide Kanäle gemischt werden. \textbf{Abbildung \ref{fig:signal}} stellt die wesentlichen Parameter der Extraktionsphase heraus.
...
Im Folgenden werden die in dieser Arbeit betrachteten Merkmale näher vorgestellt. 
 
\subsection{Merkmale}\label{sec:alg}
Merkmale können in zwei Bereiche unterteilt werden. Einmal Merkmale, die aus dem Zeitbereich stammen (\textbf{Abschnitt \ref{subsec:zeit}}) und Merkmale die aus dem Frequenzbereich stammen (\textbf{Abschnitt \ref{subsec:freq}}).\\
...
Klassifikationsalgorithmen setzen eine festgelegte Anzahl von Variablen voraus, mit welchen Musikstücke dargestellt werden. Da Mustikstücke unterschiedliche aufweisen, ist auch die Anzahl extrahierter Merkmale verschieden. Daher ist die Erstellung von Merkmalsvektoren erforderlich, die die Merkmale nach zum Beispiel statistischen Eigenschaften zusammenfasst. Hierdurch wird außerdem die zu speichernde Datenmenge pro Musikstück reduziert, welche bei mobilen Endgeräten einen nicht zu unterschätzenden Faktor darstellt.\\
Zwei in dieser Arbeit eingesetzte Methoden werden im Folgenden vorgestellt.

\subsection{Running Mean and Runnind Deviant (RMD)}

Bei dieser Prozessierungsmethode werden für jedes Merkmal anstatt den genauen Werten nur der Mittelwert und die Abweichung gespeichert. Diese Datenreduktion geschieht in zwei Schritten.\\
Im erste Schritt wird die Wertereihe eines Merkmals in gleichgroße Fenster unterteilt. Danach wird für jedes Fenster Mittelwert und Abweichung berechnet, so dass zwei gleich lange Wertereihen pro Merkmal-Element entstehen, die den Mittelwerten und Abweichungen über den Zeitverlauf entsprechen.\\
...
Diese Prozessierungsmethode basiert auf der Ermittlung von statistischen Daten aus der spektralen Dynamik eines Merkmals. Diese Methode funktioniert nur auf Merkmalen, deren Wertereihen in vektorieller Form vorliegen. Hierbei wird von D Elementen pro Vektor ausgegangen. Die Prozessierung wird in vier Schritten durchgeführt.\\
Im ersten Schritt wird jedes Elementreihe eines Merkmals in k gleichgroße Fenster unterteilt, die sich um 50\% überlappen. Anschließend mit Hilfe einer FFT in den Frequenzbereich transformiert.\\
Der zweite Schritt werden aus diesen $D\cdot k$ Fourier-Spektren Betragsspektren berechnet und diese für jedes Merkmalselement gemittelt.\\
...
Im vierten Schritt werden von allen Valley- und Contrast-Werten Mittelwert und Abweichung berechnet. Hierbei erfolgt die Berechnung sowohl zeilen- als auch spaltenweise, so dass jedes Merkmal durch vier statistische Werte repräsentiert wird.
...
\\\\
zur Vereinfachung wird angenommen, dass die Vektoren nur zwei Elemente besitzen und nur zwischen zwei Kategorien zu unterscheiden ist. Die Vektoren lassen sich im Merkmalsraum darstellen, der aufgrund der Elementanzahl ebenfalls zweidimensional ist.
%
\begin{figure}[htp]
	\centering
...
\end{figure}
%
Der SVM-Klassifikator sucht in der Trainingsphase nach einer Hyperebene, die die Vektoren unterschiedlicher Klassen mit maximalem Abstand voneinander trennt. Hierbei kann eine geeignete Ebene mehr Dimensionen aufweisen als der Merkmalsraum (vgl. \textbf{Abbildung \ref{fig:svm}}). Die gefundene Ebene wird schließlich anhand der Vektoren beschrieben, die am nächsten zur Ebene liegen. Diese werden als Support Vektoren bezeichnet. Danach kann ein zu klassifizierendes Objekt (grünes Dreieck) anhand seiner Lage zur aufgespannten Hyperebene eindeutig einer der beiden Klassen zugeordnet werden.

\subsection{Klassifikationsgüte}

Für die Bewertung eines Klassifikators wird eine Bewertungsmetrik benötigt. Als so eine Metrik wird die Klassifikationsgüte verwendet. Diese Güte gibt die Genauigkeit einer Klassifikation an und wird als Verhältnis von richtig klassifizierten Objekten zur Gesamtzahl aller klassifizierten Objekte berechnet (vgl. \textbf{Formel \ref{eqn:class}}).

\begin{equation}
\label{eqn:class}
Klassifikationsg\ddot{u}te~=~\frac{\#(richtig~klassifizierter~Objekte)}{\#(zu~klassifizierender~Objekte)}
\end{equation}

Aus der Formel ist leicht zu erkennen, dass die Klassifikationsgüte einen Wert zwischen 0 und 1 besitzt, umso näher dieser Wert an 1 liegt, desto mehr Objekte wurden richtig klassifiziert.

\subsection{Kreuzvalidierung}

Kreuzvalidierung wird für die Überprüfung eines Klassifikationsmodells herangezogen. Hierbei wird eine bereits vorklassifizierte Menge in möglichst gleichgroße Teilmengen geteilt. Anschließend wird ein Klassifikator mit N-1 dieser N Teilmengen trainiert und die N. Teilmenge klassifiziert. Dieses wird so oft wiederholt, bis jede der N Teilmengen einmal klassifiziert wurde. Die Klassifikationsgüten werden abschließend miteinander verglichen. Umso besser das aufgestellte Klassifikationsmodell ist, dest weniger weichen diese Güten voneinander ab. Außerdem weist die gemittelte Klassifikationsgüten dieser einzelnen Güten weniger Ausreißer auf.\\\\
Für die im folgenden Abschnitt angegebenen Klassifikationsgüten wurde die GTZAN Datenbank\cite{gtzan} als Grundlage verwendet. Diese beinhaltet 1000 Musikstücke (á 30 s bei 22050 Hz, 16 bit Sample, mono), die sich gleich auf die 10 enthaltenen Klassen \textit{blues, classical, country, disco, jazz, metal, pop, punk, reggae} und \textit{rock} verteilen.\\
Für die Kreuzvalidierung wurden diese 1000 Lieder in 10 Teilmengen unterteilt, so dass je 10 Lieder pro Klasse in einer Teilmenge vorhanden sind, und kein Lied doppelt vorkommt (vgl. \textbf{Tabelle \ref{tab:gtzan}}).
...
Für die Laufzeitanalysen sind vier verschiedene Methoden zur Musikklassifikation vorgegeben, die sich im wesentlichen durch die Merkmalssätze unterscheiden. Alle Methoden verwenden einen SVM-Klassifikator\cite{libsvm} und entweder die RMD-, MSA- oder eine Kombination aus beiden Prozessierungsmethoden. Die Merkmalssätze und Prozessierungsmethoden der jeweiligen angewandten Musikklassifikationsmethode sind der \textbf{Tabelle \ref{tab:meth}} zu entnehmen. Die mit diesen Methoden erreichten Klassifikationsgüten sind mit der GTZAN Datenbank\cite{gtzan} gemessen.

\begin{table}[ht]
	\centering
		\begin{tabular}{|c c|c|c|c|c|}
		\hline
		Mermalssatz & & Prozessierungsmethode & Klassifikator & Name & Güte\\
		\hline\hline
		SPC & \multirow{4}{*}{FSet1}&\multirow{4}{*}{RMD}&\multirow{4}{*}{SVM}&\multirow{4}{*}{MCL1}&\multirow{4}{*}{73,4\%}\\
		SR&&&&&\\
		SF&&&&&\\
		MFCC&&&&&\\
		\hline
		HAM & \multirow{4}{*}{FSet2}&\multirow{4}{*}{MSA}&\multirow{4}{*}{SVM}&\multirow{4}{*}{MCL2}&\multirow{4}{*}{78,6\%}\\
		NASE&&&&&\\
		OSC&&&&&\\
		MFCC&&&&&\\
		\hline
		LE & \multirow{3}{*}{FSet3}&\multirow{3}{*}{MSA \& RMD}&\multirow{3}{*}{SVM}&\multirow{3}{*}{MCL3}&\multirow{3}{*}{57,7\%}\\
		RMS&&&&&\\
		ZCR&&&&&\\
		\hline
		AOMIC& \multirow{3}{*}{FSet4}&\multirow{3}{*}{MSA \& RMD}&\multirow{3}{*}{SVM}&\multirow{3}{*}{MCL4}&\multirow{3}{*}{68,1\%}\\
		SCF&&&&&\\
		SBER&&&&&\\
		\hline
		\end{tabular}
