...
\label{ch:grund}
\rm
In diesem Kapitel werden die Grundlagen  der Audio-Signalklassifikation respektive der Musikklassifikation eingeführt.\\
...
Hamming Window ist eine andere Art der Fensterung von Signalen. Hierbei wird nicht ein quadratisches Fenster verwendet, sondern eine Kurve, die der Gauß'sche Glockenkurve ähnelt. Das Hamming Window ergibt sich nach \textbf{Formel \ref{eqn:hw}}.
...
In der Musikklassifikation liegen Audio-Samples als reelwertige Wertefolgen vor. DFT- und FFT-Algorithmen sind aber für komplexwertige Eingangsfolgen definiert. Um nun Audio-Samples mit einer DFT oder FFT zu transformieren, müssen diese in komplexwertige Folgen umdefiniert werden. Die einfachste Methode, um dieses zu erreichen, ist den Imaginär-Teil als Null anzunehmen. Dieses führt dazu, dass die sich ergebenden Fourier-Koeffizienten in der  konjugiert komplexen Achsensymmetrie aus \textbf{Formel \ref{eqn:achs}} vorliegen.
...
Dieses hat zu Folge, dass für die Weiterverarbeitung der Fourier-Koeffizienten nur die Hälfte der berechneten Koeffizienten benötigt werden. Es folgt, dass für die Berechnung einer DFT oder FFT bei reellwertigen Eingangsfolgen der Länge N eine halb so große DFT, beziehungsweise FFT, der Länge $\frac{N}{2}$ verwendet werden kann. Eine solche Transformation setzt eine anschließende Nachverarbeitung der Fourier-Koeffizienten voraus, die die erwarteten Real- und Imaginärteile in die richtige Reihenfolge bringt. Um dieses zu erreichen, muss als erstes die reellwertige Eingangsfolge $x[n]$ der Länge N mit Hilfe von \textbf{Formel \ref{eqn:comp}} in eine komplexwertige Eingangsfolge $x^+[n]$ konvertiert werden.
...
Aus der so entstandenen Eingangsfolge $x^+[n]$ können die $\frac{N}{2}$ Fourier-Koeffizienten $X^+[k]$ berechnet werden. Unter Berücksichtigung von \textbf{Formel \ref{eqn:x+}} können durch \textbf{Formel \ref{eqn:koeff}} die ersten $\frac{N}{2}$ Fourier-Koeffizienten bestimmt werden.
...
Hierfür wird zuerst der Betrag der diskreten Transformierten berechnet, welcher danach durch die Größe des betrachteten Fensters geteilt wird (\textbf{Formel \ref{eqn:aos}}).
...
Soll nun eine Frequenz $f$ in den zugehörigen Mel-Wert $m$ umgerechnen werden, wird hierfür \textbf{Formel \ref{eqn:mel}} verwendet.
...
Der Normalized Audio Spectrum Envelope (NASE) gibt Auskunft über das Energiespektrum jedes Fensters. Hierbei wird pro Fenster ein Vektor generiert, bei dem jedes Vektorelement der normalisierten, quadrierten Magnitude eines definierten Frequenz-Unterbandes entspricht.\\
...
\begin{equation}
	...
	B~=~\frac{8}{r}
\end{equation}
berechnet, wobei  für die Spektralauflösung $r$ gilt (vgl. \textbf{Abbildung \ref{fig:nase}}):
...
Beim Octave Spectral Contrast wird das Frequenzspektrum zunächst mit Bandpass-Filtern in Unterbänder aufgeteilt (siehe \textbf{Tabelle \ref{tab:osc}}). Danach werden für jedes Unterband \textit{spektrale Peaks} (\textbf{Formel \ref{eqn:peak}}) und \textit{spektrale Valleys}(\textbf{Formel \ref{eqn:valley}}) berechnet, wobei Peaks den harmonischen Komponenten und Valleys den nicht-harmonischen Komponenten oder Rauschen des Unterbandes $b$ entsprechen. Hierbei werden die Power Spektren $P_{b,n}$ vorher so sortiert, dass $P_{b,1}\geq P_{b,2}\geq ... \geq P_{b,n}$ gilt.\\
\begin{table}[ht]
	\centering
		\begin{tabular}{c | c}
...
Der Spectral Crest Factor stellt ein Maß der spektralen Flachheit eines Frequenzbandes dar und ist im Wertebereich $\left[1,\infty\right]$ definiert. Hierbei entspricht 1 maximaler Flachheit (d.h. Spektralamplituden sind gleich) und $\infty$ minimaler Flachheit (\textbf{Formel \ref{eqn:scf}}).\\
...
Im ersten Schritt wird die Wertereihe eines Merkmals in gleichgroße Fenster der Länge L unterteilt. Danach wird für jedes Fenster Mittelwert und Abweichung berechnet, so dass zwei gleich lange Wertereihen pro Merkmal-Element entstehen, die den Mittelwerten und Abweichungen über den Zeitverlauf entsprechen.\\
...
Im ersten Schritt wird jede Elementreihe eines Merkmals in k gleichgroße Fenster der Länge L unterteilt, die sich um 50\% überlappen. Diese werden mit Hilfe einer FFT in den Frequenzbereich transformiert.\\
