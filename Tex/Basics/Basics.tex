...
Hierbei werden normalerweise vorher festgelegte FeatureSets verwendet, die unter anderem aus den in \textbf{Kapitel \ref{sec:alg}} vorgestellten Algorithmen bestehen können und in der Regel drei bis vier Algorithmen umfassen. Neben den in dieser Arbeit vorgestellten Algorithmen existieren eine Vielzahl weiterer Algorithmen.\\
Für die Extraktion dieser Merkmale, im Nachfolgenden als Features bezeichnet, werden die Musikstücke als erstes in gleichgroße Fenster unterteilt. In dieser Arbeit wurde eine Fenstergröße von 512 Werten gewählt. Diese Fenster sind in der Regel nicht überlappend, müssen dieses aber nicht sein (vgl. \textbf{Kapitel \ref{subsec:fset3}}).\\
...
Klassifikationsalgorithmen erwarten, dass die Anzahl an Daten eines zu klassifizierenden Musikstücks mit der Anzahl der Daten in ihrem Referenzsets übereinstimmen. Da im Normalfall aber nicht alle Musikstücke die selbe Länge und damit mit die selbe Fensteranzahl besitzen, muss eine Datenreduktion durchgeführt werden. Dieses geschieht in der Prozessierungsphase.\\
...
In diesem Kapitel sollen die Extraktionsalgorithmen vorgestellt werden, aus denen die im \textbf{Kapitel \ref{sec:fset}} beschriebenen FeatureSets zusammengesetzt sind. Diese werden für die Laufzeitanalyse des Codes verwendet. Hierfür werden diese in zwei Bereiche unterteilt, einmal die Features, welche im Zeitbereich arbeiten (\textbf{\ref{subsec:zeit}}) und diese die im Frequenzbereich arbeiten (\textbf{\ref{subsec:freq}}).

\subsection{Notation von Ausdrücken und Variablen}\label{subsec:note}

Hier werden kurz die verwendeten Ausdrücke und Variablen eingeführt werden, die für die spätere Definition der einzelnen Features benötigt werden (\textbf{Tabelle \ref{tab:note}}). Diese Definition wird aus \cite{haller2008} und \cite{Theimer2008} übernommen.
...
Für die Laufzeitanalysen des \textit{Musikklassifikators} wurden vier verschiedene Featuresets (im späteren mit \textit{FSetN} bezeichnet) verwendet, die in den folgenden Unterkapiteln näher spezifiziert werden sollen. Diese Featuresets wurden nicht selbst erstellt, sondern so aus der Literatur übernommen.
