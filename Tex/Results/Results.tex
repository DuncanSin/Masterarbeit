...
Für weitere Diskussionen über das verbleibende Optimerungspotenzial werden nur Algorithmen betrachtet, die eine Laufzeit von über 10 $\mu s$ besitzen (vgl. \textbf{Abbildung \ref{fig:potdsp}}).
%
\begin{figure}[htp]
	\centering
...
\end{figure}
%
Bei der MAG-Berechnung sind Divison und Wurzelberechnung bereits optimiert worden. Auch die Addition wurde bereits für SPLOOP optimiert. Eine weitere Optimierung könnte einzig bei der Addition geschehen. Das Optimierungspotenzial in diese Richtung ist aber eher als gering zu bewerten.

Bei der Berechnung des MFCC-Merkmals wurde bereits die Logarithmierung optimiert. Eine weitere Optimierung wäre nur noch in Richtung DCT  und der Anwendung der Filterbank möglich. Hierbei hat die Anwendung der Filterbank mit 73,7\% den größten Anteil an der Rechenzeit. Diese ist bereits mit SPLOOP beschleunigt, so dass das Optimierungspotenzial in diese Richtung als gering zu bewerten ist.
...
\begin{figure}[htp]
	\centering
		\includegraphics[width=1\textwidth]{../Pictures/hete+arm.pdf}
	\caption{Laufzeitvergleich von ARM Cortex A8 und dem heterogenen System (unoptimiert und optimiert)}
	\label{fig:hete+arm}
\end{figure}
%
Aus der Abbildung wird ersichtlich, dass in den Vergleichen zwischen ARM und ARM+DSP der heterogene Ansatz für jede Musikklassifikationsmethode trotz Kommunikationszeit die geringeren Laufzeiten aufweist.\\
Bei der Betrachtung der Energie-Effizienz beider Ansätze ist zu sehen, dass hier ebenfalls der heterogene Ansatz die besseren Ergebnisse liefert (vgl. \textbf{Abbildung \ref{fig:enhete}}). Besonders im Fall des MCL3 übertrifft der heterogene Ansatz den ARM Cortex A8 um Längen.



\section{Fazit}\label{sec:evfaz}

Als Fazit der erhobenen Ergebnisse kann festgestellt werden, dass besonders für die Musikklassifikationsmethoden 2 und 3 sowohl die Laufzeiten als auch die Energie-Effizienzen des heterogenen Systems besser sind. Einzig bei den Werten von MCL 1 und 4 erweist sich der ARM Cortex A8 als Energie-effizienter, die Laufzeiten des heterogenen Systems übertreffen hier jedoch ebenfalls die des ARM.\\
Abschließend kann das heterogene System also als der effektivere Ansatz bewertet werden.