...
\label{ch:board}
\rm
In diesem Kapitel wird die zugrundeliegende Hardware beschrieben, auf der die Untersuchungen und Optimierungen der folgenden Kapitel durchgeführt wurden.\\
Hierfür wird als erstes das EVM8168-Entwicklungsboard der Firma Texas Instruments vorgestellt, auf welchem sich der betrachtete DaVinci\texttrademark-Prozessor befindet. Auf diesen wird danach eingegangen. In einer feineren Betrachtung werden im Anschluss die beiden im DaVinci\texttrademark-Prozessor realisierten Prozessorkerne (ARM Cortex-A8 und C674x DSP) und ihre zur Verfügung gestellten Hardwareoptimierungseinheiten vorgestellt. Als letztes wird auf die verwendete Entwicklungsumgebung, dass sogenannte EZSDK der Firma Texas Instruments eingegangen werden.
...
Wie in \textbf{Abbildung \ref{fig:neon}} zu sehen ist, besteht der NEON-Coprozessor im Wesentlichen aus aus fünf Komponenten, außerdem besitzt diese zwei Eingangsqueues. In die erste Eingangsqueue werden die vom ARM vorbereiteten NEON-SIMD- oder VPFv3-Befehle geschrieben, diese Queue kann bis zu 16 Einträge halten und wird als \textit{NEON Instruction Queue} bezeichnet. Die zweite Queue ist für das Vorladen von Daten aus dem L1- oder L2-Cache verantwortlich, diese kann 12 Einträge halten und wird als \textit{NEON Data Queue} bezeichnet. Diese Queue wird von der Load/Store-Einheit ausgelesen oder beschrieben.\\
Die in der NEON Instruction Queue befindlichen Daten werden vor der Ausführung auf dem NEON-Coprozessor nochmal mit einer eigenen Instruction Decode-Einheit dekodiert, hierbei werden auch VFPv3- und NEON-SIMD-Befehle voneinander getrennt und anschließend entweder an die NEON-SIMD-Pipeline oder die VFPv3-Einheit weitergeleitet. Hierbei ist zu Erwähnen, dass immer nur eine dieser beiden Einheiten arbeiten kann, also eine Verarbeitung von NEON-SIMD- und VFPv3-Befehlen nicht parallel möglich ist.\\
Des weiteren besitzt der NEON-Coprozessor eine physikalische Registerbank, die in 32x64-bit Registerfiles unterteilt ist. Sowohl VFPv3-Einheit als auch die NEON-SIMD-Pipeline haben Zugriff auf diese. Für diese Registerbank sind drei verschiedene Namensgebungen vorhanden, die im Assembler-Code verwendet werden können. Diese Namensgebungen bezeichnen die Wortbreite, mit der Aus den Registern gelesen werden kann. Es ist möglich mit einfacher (S-Namensgebung), doppelter (D-Namensgebung) und vierfacher (Q-Namensgebung) Wortbriete aus diesem Register zu lesen. Bei einfacher Wortbreite ist allerdings nur das halbe Register zugreifbar.\\
...
\item Mit dem MRC-Befehl ließt der ARM die Ergebnisse des NEON-Coprozessors aus, dies dauert mindestens 20 Takte
\end{enumerate}
...
Sie kann auf die in \textbf{Abbildung \ref{fig:neon}} abgebildete Registerbank mit einfacher und doppelter (S- und D-Namensgebung) zugreifen.

In \textbf{Tabelle \ref{tab:vfp}} sind wesentliche Instruktionen aus dem Instruktionssatz der VFPv3-Einheit mit Angabe der Ausführungszeit in Takten angegeben. Hierbei ist zu erwähnen, dass diese Ausführungszeiten datenabhängig sind.
...
Im Wesentlichen besteht der C674x aus vier Komponenten. Die erste Komponente ist die C674x+ CPU in der Befehle des C674x-Instuktionssatz verarbeitet werden. Neben dieser bestehen getrennte L1-Befehls- (L1P) und Datencaches (L1D) und ein L2-Cache. Dem L1D-Cache ist es bei Bedarf möglich mit 8x32bit-Datenleitungen auf den L1D-RAM zuzugreifen. In \textbf{Tabelle \ref{tab:dspcache}} sind die wesentlichen Merkmale der verwendeten Caches angegeben.
...
Auch wenn einige Instruktionen auf mehreren Ausführungseinheiten ausgeführt werden können, lässt sich eine grobe Zuordnung von Instruktionen zu Asuführungseinheiten feststellen: 
\begin{itemize}
...
In der Execute-Phase werden die vorher geladenen und decodierten Befehle auf den Hardwareeinheiten ausgeführt. Diese Stufe wird  in zehn Stufen unterteilt, E1-E10. Die Anzahl durchzulaufender Stufen ist durch instruktionsbezogene Delayslots spezifiziert.
...
Texas Instruments bietet zu den DSPs der C6000-Reihe verschiedene Bibliotheken mit achritekturspezifisch optimierten Algorithmen an. Einige dieser optimierten Bibliotheken sind im Umfang des EZSDK (siehe \textbf{Kapitel \ref{sec:ezsdk}}) enthalten. Im Folgenden werden die DSPLIB  und zum anderen die MATHLIB vorgestellt.
...
Einbindung von NEON-SIMD-Instruktionen in den Programm-Code drei Wegen erfolgen. Es ist dem Compiler möglich selber Stellen des Programm-Codes zu identifizieren, die durch die NEON-SIMD-Einheit beschleunigt werden können. \textbf{Listing \ref{code:spfc}} und \textbf{Listing \ref{code:spfneon}} zeigen ein Beispiel in dem die Multiplikation durch NEON-SIMD-Assembler parallelisiert wird.
\newpage 
...
In diesem Fall gibt es die Möglichkeit NEON-Intrinsics im Programm-Code zu verwenden. Hierbei werden Berechnungen durch entsprechende NEON-Intrinsics ersetzt.\textbf{Listing \ref{code:firstc}} - \textbf{\ref{code:firstneon}} zeigen ein Beispiel in dem die Addition innerhalb der Schleife durch NEON-Intrinsics für vier parallele Addtitionen ersetzt wurden.
...
Dieses bieten den Vorteil, dass durch relativ geringen Aufwand Schleifen parallelisiert werden können und der Datenfluss definiert werden kann. Allerdings wird ein Overhead durch Lade und Speicheroperationen erzeugt.\\
...
Außerdem stellt C6EZRun Compiler für diese beiden Versionen bereit.\\
Bei C6Runlib werden Programm-Codes auf dem als Funktionen durch den ARM aufgerufen. Dies bedeutet, dass der ARM erst nach Ende der DSP-Programm-Code Ausführung mit der eigenen Ausführung fortfahren kann. \textbf{Abbildung \ref{fig:c6lib}} verdeutlicht dieses Prinzip.\\
%
\begin{figure}[htb]
	\centering
...
\end{figure} 
%
C6Runapp ermöglicht hingegen die Ausführung von ganzen Programmen auf dem DSP. Hierbei werden DSP-Programme von der Linux-Console des ARMs aufgerufen, als ob sie auf diesem ausgeführt würden. Der ARM fungiert hierbei als Initiator, außerdem ist es dem DSP so möglich Ausgaben auf der Console des ARM zu tätigen. Eine gleichzeitige Ausführung eines zweiten Programms auf dem ARM ist möglich, allerdings sind keine Protokolle zur Kommunikation dieser beiden Programme definiert und müssen gegebenenfalls geschrieben werden. \textbf{Abbildung \ref{fig:c6app}} verdeutlicht dieses Prinzip.\\ 
%
\begin{figure}[htb]
	\centering
...
Da die beiden Compiler aus dem C6EZRun-Framework auf dem C6000-DSP-Compiler aufsetzt, können auch die Compileroptimierungen des C6000-Compilers verwendet werden, Diese Optimierungsstufen optimieren auf Geschwindigkeit.\\
