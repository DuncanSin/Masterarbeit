...
Der DM816x ist ein heterogener Prozessor, der aus mehreren Subsystemen und Coprozessoren besteht.
...
\begin{itemize}
\item ARMv7-A Prozessorkern (\textbf{\ref{subsubsec:armv7}})
\item NEON-Coprozessor (\textbf{\ref{subsubsec:neon}})
\end{itemize}
...
Wie schon erwähnt, kann dieser Prozessorkern nur Integeroperationen bearbeiten, wird aber bei der Kompilierung eines Programms für den Cortex-A8 dem Compiler nicht die Benutzung einer Floating Point-Einheit vorgeschrieben (vgl. \textbf{Kapitel \ref{subsubsec:neon}}) werden auch entsprechende Befehle auf den Integeroperationen des ARMv7-A Prozessorkerns emuliert, was zu langen Ausführungszeiten führen kann. Derartige Floating Point Instruktionen sind durch das Präfix \textit{\_\_aeabi\_} im Assembler-Code gekennzeichnet. Dieses bedeutet, dass Floating Point-Operationen nicht auf Hardware-, sondern auf Softwareebene berechnet werden.\\
Die \textbf{Tabelle \ref{tab:aeabi}} gibt einen Überblick über die in Software hinterlegten Floating Point-Operationen des ARMv7-A Prozessorkerns.\\
%
\begin{table}[h]
\centering
\begin{tabular}{|c|c|}
...
\begin{enumerate}
\item NEON-SIMD- und VFPv3-Befehle durchlaufen die gesamte ARM-Pipeline und werden am Ende der Execute-Phase in eine NEON Instruction Queue geladen, welche 16 Einträge halten kann. Danach gelten die NEON- und VFPv3-Befehle für den ARM als abgearbeitet und er kann wieder ARM-Befehle ausführen. Eine Ausnahme bildet hier der Fall, dass die NEON Instruction Queue voll ist. Sollte dieses geschehen muss der ARM warten bis wieder Einträge in diese Queue geschrieben werden können.
...
\begin{itemize}
\item Der ARM Prozessor schreibt mit dem MCR-Befehl Daten in die NEON Daten Queue, welche 12 Einträge halten kann
...
Die VFPv3-Einheit stellt einen Coprozessor zur Berechnung von Floating Point-Zahlen zur Verfügung und unterstützt den \textit{ANSI/IEEE Standard 754-1985} in vollem Umfang, welcher auf der \textit{VFPv3-Architektur} basiert.\\
...
Sie hat auf die in \textbf{Abbildung \ref{fig:neonbank}} abgebildete Registerbank wie folgt Zugriff:
...
Diese Einheit basiert auf der \textit{Advanced SIMD Media Processing-Architektur}. SIMD steht hierbei für \textit{Single Input Multiple Data} und heißt, dass ein und der selbe Befehle gleichzeitig auf mehrere Eingangsdaten angewendet werden kann. Der NEON-SIMD-Einheit ist es so möglich pro Operand 128 Bit gleichzeitig einzulesen und zu schreiben, allerdings kann sie nur 64 Bit gleichzeitig verarbeiten, woraus folgt, dass immer zwei Operationen parallel ausgeführt werden können.\\
...
\end{itemize}
Da diese vier Optionen für die spätere Optimierung verwendet werden, wird zu jeder dieser Varianten auch ein Beispiel gegeben. Wichtig ist, dass bei allen vier Varianten das Compilerflag \textit{-mfpu=neon} gesetzt sein muss.
...
In \textbf{Listing \ref{code:firstneon}} sind alle Instruktionen wiederzufinden, die in \textbf{Listing \ref{code:firstcv}} definiert wurden.
\newpage
...
Hierbei werden immer zwei Berechnungen parallel durchgeführt. Jedes BF() in \textbf{Listing \ref{code:fft4c}} besteht aus einer Addition und einer Subtraktion, es werden also insgesamt acht Additionen und 8 Subtraktionen durchgeführt. Im NEON-Assembler-Code aus \textbf{Listing \ref{code:fft4neon}} befinden sich vier VADDs und vier VSUBs, die jeweils für 2 parallele Berechnungen stehen. Also werden auch hier wieder acht Additionen und acht Subtraktionen durchgeführt.
...
\textbf{Abbildung \ref{fig:Register}} zeigt den Aufbau der beiden Datenpfade Register File A und B, nicht eingezeichnet sind die beiden sogenannten Crosspaths, die es Operationen aus dem Datenpfad A erlauben ein Datum pro Takt aus Register File B zu lesen und umgekehrt. Diese Crosspaths sind im Assembler-Code durch ein X hinter der Ausführungseinheit gekennzeichnet.Insgesamt besitzt der DSP 64 32 Bit-Register, je 32 in A und B.
Auch wenn einige Befehle auf mehreren Ausführungseinheiten ausgeführt werden können, lassen sich diese wie folgt grob unterteilen: 
\begin{itemize}
...
In der Fetch-Phase werden Befehle aus dem Instruktionscache geladen. Diese ist, wie bereits erwähnt, in vier Stufen unterteilt:
...
In der Execute-Stufe werden die vorher geladenen und decodierten Befehle auf den Hardwareeinheiten ausgeführt. Diese Stufe wird formell in zehn Stufen unterteilt, E1-E10, in denen je nach Befehl Lade- und/oder Schreiboperationen ausgeführt werden. Hierbei kann es allerdings passieren, dass bei manchen Befehlen eine bestimmte Zeit vergeht, bis das Ergebnis am Ausgang anliegt, diese Zeit wird als Delayslots bezeichnet, welche ab der Phase E1 gezählt werden (vgl. \textbf{Tabelle \ref{tab:c674x}}). Die maximale Anzahl an Delayslots im Instruktionssatz des C674x beträgt 9.\\
...
Hier kommt die Software Piplined Loop ins Spiel.\\ 
...
Wie durch den Befehl \textit{SPLOOPD 4} angegeben ist, beträgt die Länge eines Iterationsintervals vier Takte. Des weiteren sind sechs Stages vorhanden (innerhalb des Codes durch Leerzeilen getrennt), die jeweils vier Takte lang sind. Die einzige Ausnahme ist der Befehl \textit{LDW}, dieser wird in fünf Takten ausgeführt, da allerdings nicht direkt nach dem 5. Takt aus A5 gelesen wird, stellt dieses kein Problem dar. Der Befehl \textit{SPKERNEL} markiert das Ende des Kernels, die Argumente dieses Befehls geben an, wie lange nach dem letzten Durchlauf gewartet werden muss, bis wieder Operationen ausgeführt werden können, in diesem Fall muss die Laufzeit einer Stage und 2 Takte gewartet werden, also insgesamt 6 Takte.\\
Die Konstante \textit{scsize} aus \textbf{Listing \ref{code:scc}} steht für die halbe Breite eines betrachteten Fensters.\\
Die Berechnung der Gesammtlaufzeit einer Schleife mit SPLOOP wird nach \textbf{Formel \ref{eqn:sploop}} berechnet. Im Fall des \textit{Spectral Centroids} mit einer Fensterbreite von 512, beträgt die Gesamtlaufzeit in Takten 976.
...
In diesem Kapitel sollen die Optimierungsstufen des DSP-Compilers aus dem C6EZRun-Framework (siehe \textbf{Kapitel \ref{sec:c6run}}) erläutert werden.\\
...
\begin{itemize}
\item Die Inline Calls zu \textit{sqrt} und \textit{\_\_c6xabi\_sqrt}
...
\paragraph{DSPLIB}$\;$ \\\\
Wie schon erwähnt ist die DSPLIB-Bibliothek eine Zusammenfassung von Algorithmen aus der Signalverarbeitung, die für den Gebrauch auf DSPs der C6000-Reihe optimiert wurde und direkt von Texas Instruments stammt. Diese liegen innerhalb dieser Bibliothek sowohl als C- als auch als Assembler-Code vor. Unter anderem enhält sie auch optimierte FFTs.
