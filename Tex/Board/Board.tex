...
Diese beiden Prozessorkerne teilen sich Instruction Fetch, Instruction Decode, Instruction Execute, Load/Store und L2 Cache\cite{cortexa8}.\\
...
In \textbf{Tabelle \ref{tab:neon}} ist ein Auszug des NEON-SIMD-Befehlssatzes mit Ausführungszeiten in  Takten angegeben. Sigle-Precision-Befehle sind im späteren Assemblercode durch den Zusatz \textit{.f32} hinter dem Befehlsnamen gekennzeichnet\cite{cortexa8}.
...
Die Fetch-Phase besitzt vier Stufen, die Decode-Phase zwei Stufen und die Execute-Phase maximal 10 Stufen (vgl. \textbf{Abbildung \ref{fig:dsppipe}}), hierbei ist eine Stufe immer einen Takt lang\cite{sprufe8b}.
%
\begin{figure}[htb]
	\centering
...
Zum Laden von Befehlen aus dem Instruktionscache wird ein sogenanntes Fetch Packet (FP) verwendet, welches aus acht Befehlen besteht. In einem Fetch Paket sind neben den acht Befehlen auch noch Informationen darüber enthalten, welche Befehle parallel ausgeführt werden können\cite{sprufe8b}.
...
Wenn Schleifen optimiert werden sollen gibt es neben Loop-Unrolling meistens keine andere Möglichkeit. Dieses Unrolling nutzt aber nicht effektiv alle acht Ausführungseinheiten aus.
...
Das LBC fungiert als Inhaltsverzeichnis für den Loop Buffer und gibt an, welcher Stage ausgeführt werden muss. Das LBC wird in zwei Fällen auf null zurückgesetzt. Entweder wenn ein Befehl aus der SPLOOP(D/W)-Befehlsgruppe ausgeführt wird, oder wenn der Iterationsinterval-Wert erreicht ist, der beim letzten Befehl der eben genannten Gruppe mit übergeben wurde. Sollte der zweite Fall eintreten, wird das ILC um 1 dekrementiert.\\
...
Im ILC ist die Gesamtzahl der Durchläufe hinterlegt, die für die Ausführung der Schleife erforderlich sind. Erreicht das LBC den Iterationsinterval-Wert, wird das ILC dekrementiert, erreicht das ILC den Wert 0, endet die SPLOOP.
...
Dieser besitzt neben den gängigen Optimierungsstufe O0-O3, auch die Möglichkeit eine softwarebasierte Pipelinestruktur zu verwenden, die sich Software Pipelined Loop oder kurz SPLOOP nennt\cite{spru187o}.
...
Texas Instruments bietet zu seinen DSPs der C6000-Reihe auch noch eine Reihe Bibliotheken an, diese enthalten Algorithmen die speziell für DSPs dieser Baureihe optimiert wurden. Im Folgenden sollen zwei dieser Bibliotheken näher vorgestellt werden, zum einen die DSPLIB mit Algorithmen aus der Signalverarbeitung und zum anderen die MATHLIB mit optimierten mathematischen Funktionen.

\paragraph{DSPLIB}$\;$ \\\\
Wie schon erwähnt ist die DSPLIB-Bibliothek eine Zusammenfassung von Algorithmen aus der Signalverarbeitung, die für den Gebrauch auf DSPs der C6000-Reihe optimiert wurde und direkt von Texas Instruments stammt. Diese liegen innerhalb dieser Bibliothek sowohl als C- als auch als Assembler-Code vor. \textbf{Tabelle \ref{tab:dsplib}} zeigt einen Auszug der vorhandenen Algorithmen und ihre Funktionsnamen innerhalb der Bibliothek.

\begin{table}[ht]
\centering
\begin{tabular}{|c|c|}
\hline
Funktionsname & Alorithmus\\
\hline\hline
DSPF\_sp\_autocor & Autocorrelation\\
DSPF\_sp\_biquad & Biquad Filter\\
DSPF\_sp\_blk\_move & Block Copy\\
DSPF\_sp\_convol & Convolution\\
DSPF\_sp\_dotp\_cplx & Complex Dot Product\\
DSPF\_sp\_dotprod & Dot Product\\
DSPF\_sp\_fftSPxSP & Mixed Radix Forward FFT mit Bit Reversal\\
DSPF\_sp\_fir\_cplx & Complex FIR Filter\\
DSPF\_sp\_fir\_gen & FIR Filter\\
DSPF\_sp\_fir\_r2 & FIR Filter Alternate Implementation\\
DSPF\_sp\_fircirc & FIR Filter mit Circular Input\\
DSPF\_sp\_ifftSPxSP & Mixed Radix Inverse FFT mit Bit Reversal\\
DSPF\_sp\_iir & IIR Filter\\
DSPF\_sp\_irrlat & Lattice IIR Filter\\
DSPF\_sp\_lms & LMS Adaptive Filter\\
DSPF\_sp\_mat\_mul & Matrix Multiply\\
DSPF\_sp\_mat\_mul\_cplx & Complex Matrix Multiply\\
DSPF\_sp\_mat\_trans & Matrix Transpose\\
DSPF\_sp\_maxidx & Maximum Index\\
DSPF\_sp\_maxval & Maximum Value\\
DSPF\_sp\_minerr & VSELP Vocoder Codebook Search\\
DSPF\_sp\_minval & Minimum Value\\
DSPF\_sp\_vecmul & Vector Multiplication\\
DSPF\_sp\_vecrecip & Vectro Reciprocal\\
DSPF\_sp\_vecsum\_sq & Vector Sum of Squares\\
\hline
\end{tabular}
\caption{Auszug der Algorithmen der DSPLIB und ihre Funktionsnamen\cite{dsplib}}
\label{tab:dsplib}
\end{table}

\paragraph{MATHLIB}$\;$ \\\\

Die MATHLIB-Bibliothek ist ebenfalls für die Verwendung auf DSPs der C6000-Reihe optimiert. Auch diese Bibliothek stammt von Texas Instruments. Auch hier liegen der Quellcode sowohl als C- als auch als Assembler-Code vor.\\
Die mathematischen Funktionen sind nur für Double-Precision vorhanden, bieten aber neben der Option einzelne Aufrufe auszuführen, bei denen auch Floatzahlen als Argumente verwendet werden können, die Möglichkeit Operationen auf Arrays auszuführen. Werden diesen Arrayoperationen zum Beispiel die Array a und b übergeben, wird immer das n-te Element von a mit dem n-ten Element von b verrechnet. Da alle Operationen auf Double-Precision ausgelegt sind, werden bei Arrayoperationen allerdings nur Eingabearray vom Datentyp \textit{double} akzeptiert, was für Single-Precision-Berechnungen eine manuelle Konvertierung von Floatarrays in Doublearrays erfordert. 